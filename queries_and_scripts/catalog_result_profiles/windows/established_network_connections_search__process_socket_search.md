# Process Socket Search

## Catalog Identity

- Catalog ID: `established_network_connections_search`
- Catalog name: `Process Socket Search`
...
- Returned labels:
- `process_socket_search`

Observed columns:

| Label | Columns |
| --- | --- |
| `process_socket_search` | `name`, `fd`, `socket`, `family`, `protocol`, `local_address`, `remote_address`, `local_port`, `remote_port` |

Label row counts:
- `process_socket_search`: 8
...
Catalog warnings:
- This query does not include connections on loopback interfaces.
- This query does not include listening sockets.
