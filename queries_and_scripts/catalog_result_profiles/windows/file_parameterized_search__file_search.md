...
- Catalog ID: `file_parameterized_search`
- Catalog name: `File Search`
...
| Label | Columns |
| --- | --- |
| `file_search` | `directory`, `filename`, `uid`, `gid`, `mode`, `device`, `size`, `atime`, `mtime`, `ctime`, `btime`, `hard_links`, `symlink`, `file_id`, `sha256` |
...
The endpoint answered and returned 4659 rows in the validation run.
...
Catalog warnings:
- This query can produce excessive amounts of data and results can take time depending on the number and size of files being queried. Careful selection of parameters is advised.
