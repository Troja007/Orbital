# Windows Mofcomp Loading MOF File Monitoring

## Catalog Identity

- Catalog ID: `windows_mofcomp_loads_mof_monitoring`
- Catalog name: `Windows Mofcomp Loading MOF File Monitoring`
- Platform: windows
...
Endpoint answered with no matching event rows in the available log scope. This is a no-hit for the exact event condition, not proof the behavior never happened.
...
Catalog warnings:
- This query searches process data from Windows eventlog's audit logs. If the Windows's process event/audit logging is disabled, then this query will not return any data even if this activity occured on the host.
