# Special Privileges Logons Monitoring

## Catalog Identity

- Catalog ID: `special_privileges_logons_search`
- Catalog name: `Special Privileges Logons Monitoring`
- Platform: windows
...
- Returned labels:
- `special_privileges_logons_search`

Observed columns:

| Label | Columns |
| --- | --- |
| `special_privileges_logons_search` | `TimeGenerated`, `Message`, `Subject_User_Name`, `Privilege_List`, `EventIdentifier`, `SourceName`, `Logfile`, `TimeWritten`, `Type` |

Label row counts:
- `special_privileges_logons_search`: 22

## How To Read The Result

Observed validation result:

The endpoint answered and returned 22 rows in the validation run.

If rows are returned:

Endpoint answered with event rows. Interpret as matching events present in the available log scope; validate timing, event semantics, and surrounding activity.
...
Catalog warnings:
- This query can result in excessive amounts of data. Additional conditional clauses for reducing data size and increasing relevance are advised.
