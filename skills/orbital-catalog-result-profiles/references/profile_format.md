# Catalog Result Profile Format

Use this reference when changing the generated result-profile structure or reviewing generated Markdown files.

## Purpose

Each per-Catalog-ID Markdown file should help an incident responder answer:

- What does this Orbital catalog query answer?
- What does a returned row mean?
- What does zero rows mean?
- What does no endpoint response mean?
- What assumptions are safe?
- What assumptions are unsafe?
- What follow-up is appropriate?
- What does representative sanitized result data look like?

Do not optimize these files for developers only. They are analyst-facing explanation artifacts.

## Required Per-Query Sections

Generate each file with these sections:

1. H1 catalog query title
2. `Catalog Identity`
3. `What This Query Answers`
4. `Expected Result Shape`
5. `How To Read The Result`
6. `Incident Responder Assumptions`
7. `Caveats`
8. `Recommended Follow-Up`
9. `Explanation Template`
10. `Privacy Boundary`
11. `Sample Result Data`

`Sample Result Data` must be the final section in the generated Markdown file.

## Result Reading Rules

Separate these states clearly:

- Endpoint answered with rows: interpret returned rows according to catalog purpose, categories, columns, and warnings. Rows are evidence, not automatically a malicious verdict.
- Endpoint answered with zero rows: read as no matching rows for the exact query scope. This is not a query failure and not proof the behavior never happened.
- Endpoint did not respond: do not treat as zero rows or clean. Check target selection, endpoint online state, job status, result retrieval, and API errors before rerunning.
- Completed with label or endpoint errors: read error fields before interpreting row counts.

## Sample Result Data

For profiles generated from validation output with rows, append a small sanitized sample at the end of each per-Catalog-ID Markdown file.

Sampling rules:

- Prefer 1-3 representative rows per returned label.
- Do not show more than 15 sanitized sample rows total in a single per-Catalog-ID Markdown profile, even when the response includes many more rows.
- When multiple labels return rows, allocate the 15-row maximum in deterministic label-sorted round-robin order, taking up to three rows per label before moving to the next round.
- Keep column names intact.
- Keep safe structural hints where they help interpretation, such as timestamp format, hash length, registry hive family, file-extension class, process-name class, or status/state category.
- Do not include full raw values when they identify a person, endpoint, tenant, customer, target, validation run, or credential.
- If a row cannot be sanitized safely, omit it and state that sample data was omitted for privacy.
- Use default deny for sample values. Emit only approved structural placeholders; redact unknown fields, nested values, and unrecognized strings rather than copying them.

Redaction rules:

- Replace usernames with `<redacted:username>`.
- Replace user profile paths with a neutral equivalent such as `C:\Users\<redacted:username>\...`.
- Replace hostnames with `<redacted:hostname>`.
- Replace IP addresses with `<redacted:ip>`.
- Replace GUIDs with `<redacted:guid>`.
- Replace customer, tenant, organization, or target selector values with `<redacted:sensitive-value>`.
- Replace Job IDs, `orbital_queryID` values, API URLs, tokens, secrets, bearer strings, client IDs, and client secrets with `<redacted:sensitive-value>`.
- Replace raw endpoint and label error text with generic sanitized error messages or error classes.

## Filename Rule

Use:

```text
<catalog_id>__<normalized_query_name>.md
```

Normalization:

- lowercase
- ASCII letters and digits
- replace non-alphanumeric runs with `_`
- trim leading/trailing `_`

Example:

```text
process_hashes__sha256_hash_of_running_processes.md
```

## Machine And Human Files

Keep both forms:

- `windows_stock_catalog_result_profiles.jsonl`: machine-readable index and automation source. It may contain sample availability and counts, but never sample-row values.
- `windows_stock_catalog_result_profiles.md`: human-readable summary table. It never contains sample-row values.
- `windows/*.md`: analyst-facing per-query explanation files.

The JSONL and per-query Markdown should be generated by the same helper so they stay consistent.

## Privacy Boundary

Allowed:

- Catalog metadata
- Sanitized validation status
- Row counts and buckets from the validation run
- Returned label names
- Returned column names
- Sanitized error classes
- Generic target scope, such as "one explicit Windows endpoint"
- Sanitized sample result rows in the final per-Catalog-ID Markdown section, capped at 15 rows per profile
- Sample availability, sanitized sample-row count, and omitted-row count in JSONL

Forbidden:

- Raw endpoint result rows
- Hostnames
- Target selectors such as `host:<value>`
- Orbital Job IDs / `orbital_queryID`
- Usernames
- IP addresses
- GUIDs
- Tenant or customer identifiers
- Raw API responses
- Credentials, bearer tokens, client IDs, or secrets
- Sample-row values in the JSONL index or aggregate summary Markdown

## Validation Commands

Run from the project root:

```bash
PYTHONPYCACHEPREFIX=/tmp/orbital_pycache python3 -m py_compile tools-and-memory/generate_catalog_result_profiles.py
python3 tools-and-memory/generate_catalog_result_profiles.py
python3 tools-and-memory/generate_catalog_result_profiles.py --validate-only
find queries_and_scripts/catalog_result_profiles/windows -type f -name '*.md' | wc -l
```

The validation command fails when a JSONL profile contains sample rows, a per-Catalog-ID file does not end with `Sample Result Data`, a sample exceeds 15 rows, or a sample cell is not a sanctioned structural placeholder. If it fails, stop before staging. Fix the generator or generated files, rerun generation, and validate again.
