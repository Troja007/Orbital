...
def configure_error_context(args: argparse.Namespace, task: dict[str, str]) -> None:
    ERROR_CONTEXT.clear()
...
def set_error_phase(phase: str, **context: object) -> None:
    ERROR_CONTEXT["phase"] = phase
    ERROR_CONTEXT.update({key: value for key, value in context.items() if value is not None})


def error_text(value: object) -> str:
    if isinstance(value, str):
        return value.strip()[:1000]
    return json.dumps(value, sort_keys=True, ensure_ascii=True)[:1000]
...
def append_error_event(
    *,
    ...
    orbital_query_id: object = None,
) -> None:
    meaningful = meaningful_error(error)
    if not meaningful:
        return
    ledger_path = str(ERROR_CONTEXT.get("run_ledger") or "")
    if not ledger_path:
        return
    message = error_text(meaningful)
    ...
    if label:
        record["query_label"] = label
    if http_status is not None:
        record["http_status"] = http_status
    if orbital_query_id:
        record["orbital_queryID"] = orbital_query_id
    for key in ("region", "query_mode", "api_path"):
        if ERROR_CONTEXT.get(key):
            record[key] = ERROR_CONTEXT[key]
    task = ERROR_CONTEXT.get("task")
    if isinstance(task, dict):
        record |= task
    try:
        append_run_ledger(ledger_path, record)
    except OSError:
        # Do not replace the original API or endpoint error with a local logging error.
        pass


def response_error_events(payload: dict) -> list[dict[str, object]]:
    events: list[dict[str, object]] = []
    top_level_error = meaningful_error(payload.get("errors"))
    if top_level_error:
        events.append({"scope": "api_response", "error": top_level_error})
    for result in payload.get("results") or []:
        if not isinstance(result, dict):
            continue
        endpoint_error = meaningful_error(result.get("error"))
        if endpoint_error:
            events.append({"scope": "endpoint", "error": endpoint_error})
        for query_result in result.get("queryResult") or result.get("osQueryResult") or []:
            if not isinstance(query_result, dict):
                continue
            query_error = meaningful_error(query_result.get("error"))
            if query_error:
                events.append(
                    {
                        "scope": "endpoint_osquery",
                        "label": str(query_result.get("label") or ""),
                        "error": query_error,
                    }
                )
    return events
...
def record_response_errors(payload: dict, *, phase: str, orbital_query_id: object = None) -> None:
    seen: set[tuple[str, str, str]] = set()
    for event in response_error_events(payload):
        message = error_text(event["error"])
        key = (str(event["scope"]), str(event.get("label") or ""), message)
        if key in seen:
            continue
        seen.add(key)
        append_error_event(
            ...
            error=event["error"],
            orbital_query_id=orbital_query_id,
        )


def record_foundational_error(exc: Exception, http_status: int | None = None) -> None:
    append_error_event(
        phase=str(ERROR_CONTEXT.get("phase") or "initialization"),
        scope="foundational",
        error=str(exc),
        http_status=http_status,
        orbital_query_id=ERROR_CONTEXT.get("orbital_queryID"),
    )
...
def public_context_summary(
    org_context: dict[str, object], credential_context: dict[str, object]
) -> dict[str, object]:
    orgs = org_context.get("orgs")
    return {
        "org_context_source": org_context.get("source") or "unset",
        "org_group_selected": bool(str(org_context.get("org_group") or "").strip()),
        "org_identifier_count": len(orgs) if isinstance(orgs, list) else 0,
        "credential_source": credential_context.get("source") or "unset",
        "credential_env_file_loaded": bool(str(credential_context.get("env_file") or "").strip()),
    }
...
def fetch_access_token(region: str, timeout: int) -> str:
    set_error_phase("token_request", region=region)
    existing = first_env(
        [
            "ORBITAL_API_TOKEN",
            "ORBITAL_TOKEN",
...
def meaningful_error(value: object) -> object:
    """Return a non-empty Orbital error value, or None for empty placeholders."""
    if value is None:
        return None
    if isinstance(value, str):
    ...
    if isinstance(value, list):
        cleaned = [item for item in value if meaningful_error(item)]
        return cleaned or None
    return value
...
def parse_rows(payload: dict) -> tuple[list[dict], list[dict]]:
    ...
    for result in payload.get("results") or []:
        ...
        for query_result in result.get("queryResult") or []:
            query_error = meaningful_error(query_result.get("error"))
            if query_error:
                result_rows.append({"error": query_error, "host": host, "nodeId": node_id})
                continue
            ...
            for index in range(0, len(values), width):
                ...
                if len(chunk) == width:
                    result_rows.append(dict(zip(columns, chunk)))
        if result_rows:
            rows.extend(result_rows)
        else:
            endpoint_error = meaningful_error(result.get("error"))
            if endpoint_error:
                rows.append({"host": host, "nodeId": node_id, "error": endpoint_error})
    return rows, meta
...
def parse_any_rows(payload: dict) -> tuple[list[dict], list[dict]]:
    ...
    for result in payload.get("results") or []:
        ...
        reported = result.get("reported")
        meta.append({"host": host, "nodeId": node_id, "reported": reported})
        result_rows = parse_result_rows(result)
        if result_rows:
            rows.extend(result_rows)
        else:
            endpoint_error = meaningful_error(result.get("error"))
...
def parse_immediate_rows(payload: dict) -> tuple[list[dict], list[dict]]:
    ...
    legacy_rows, legacy_meta = parse_rows(payload)
    if legacy_rows:
        return legacy_rows, legacy_meta
    stored_rows, stored_meta = parse_any_rows(payload)
    if stored_rows or stored_meta:
        return stored_rows, stored_meta or legacy_meta
    return legacy_rows, legacy_meta
            ...
            query_summaries.append(
                {
                    "label": query_result.get("label"),
                    "row_count": row_count,
                    "error": meaningful_error(query_result.get("error")) or "",
        ...
        summaries.append(
            {
                ...
                "endpoint_error": endpoint_error.get("en")
                if isinstance(endpoint_error, dict)
                else endpoint_error,
...
def fetch_job(region: str, token: str, job_id: str, timeout: int, results: bool) -> dict:
    set_error_phase(
        "job_results_fetch" if results else "job_status_fetch",
        region=region,
...
def should_fetch_results_after_poll(polls: list[dict]) -> bool:
    if has_completed_endpoint(polls):
        return True
    done_counts = [poll.get("done_count") for poll in polls]
    if done_counts and all(isinstance(count, int) for count in done_counts):
        return max(done_counts) > 0
    return True
...
def main() -> int:
    ...
    configure_error_context(args, task)
    set_error_phase("mapping_resolution")
    codex_mapping = load_codex_org_mapping(args.org_mapping_state_file)
    ...
    set_error_phase("org_context_resolution", region=region)
    validate_org_context_against_mapping(args, codex_mapping)
    org_context = resolve_org_context(args, region)
    set_error_phase("credential_resolution", region=region)
    credential_context = resolve_credential_context(args, codex_mapping)
    context_summary = public_context_summary(org_context, credential_context)
    ...
    if args.job_id:
        ...
        if args.raw_response:
            output["org_context"] = org_context
            output["credential_context"] = credential_context
        ...
        if isinstance(payload, dict):
            record_response_errors(
                payload,
                phase="job_results_response" if args.job_results else "job_status_response",
                orbital_query_id=args.job_id,
            )
    ...
    ERROR_CONTEXT["orbital_queryID"] = orbital_query_id
    record_response_errors(payload, phase="query_submission_response", orbital_query_id=orbital_query_id)
    ...
    if not orbital_query_id:
        ...
        append_error_event(
            phase="job_id_extraction",
            scope="foundational",
            error=f"POST {api_path} response did not expose a Job ID.",
        )
    if orbital_query_id and not args.no_status_poll:
        try:
            ...
            if not data_rows and not query_errors and should_fetch_results_after_poll(polls):
...
if __name__ == "__main__":
    try:
        ...
        record_foundational_error(exc, http_status=exc.code)
        print(json.dumps({"status": exc.code, "error": str(exc)}, indent=2))
        raise SystemExit(2)
